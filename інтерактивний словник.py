...
while True:
    interface = buttonbox("Виберіть дію: ", "Словник", ["Додати", "Видалити", "Екзамен", "Переклад", "Вихід"],'1.gif')
    if interface == "Додати":
        ...
        file.close()
        msgbox('Done',image='4.gif')
    elif interface == "Переклад":
        word = enterbox("Напишіть своє слово")
        file = open(path1, 'r')
        for line in file:
            if line.startswith(word + " ") or line.endswith(word + "\n"):  # якщо інгліш слово ми ввели
                msgbox(line.replace(word, ""),image='5.gif')  # ми заміняємо інгліш слово на пустоту у файлі і виводиться тільки укр
        file.close()
        ...
        if mode == "En":
            ...
            for line in readFile:
                ...
                if pitania == words[0]:
                    rate += 1
        msgbox(f"Ви вгадали: {rate} слова з {len(readFile)}!",image='3.gif')
    else:
        break